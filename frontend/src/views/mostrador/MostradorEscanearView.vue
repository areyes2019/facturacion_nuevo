...
import { pedidoDeCodigoEtiqueta, vibrarLectura } from '../../lib/lectorQr'
import AppLayout from '../../layouts/AppLayout.vue'
import EscanerQr from '../../components/EscanerQr.vue'

/**
 * El escáner de etiquetas (ver 029-pwa-mostrador.md).
 *
 * Hoy, para entregar un pedido, hay que salir a la app de cámara del celular, que abre la dirección
 ...
 * diez segundos, exactamente como manda 027. Esta pantalla cambia cómo se llega, no lo que pasa al
 * llegar.
 */
...
async function onCodigo(codigo: string) {
  const pedidoId = pedidoDeCodigoEtiqueta(codigo, window.location.origin)
  ...
  await router.push({ name: 'pedidos-entregar', params: { id: pedidoId } })
}
</script>
