...
test('el listado ordena por utilidad porcentaje efectiva en ambas direcciones (035)', function () {
    $user = User::factory()->create();
    $catalogo = Catalogo::factory()->for($user)->create();

    Articulo::factory()->for($user)->for($catalogo)->create(['nombre' => 'Baja', 'utilidad_porcentaje' => 10]);
    Articulo::factory()->for($user)->for($catalogo)->create(['nombre' => 'Media', 'utilidad_porcentaje' => 25]);
    Articulo::factory()->for($user)->for($catalogo)->create(['nombre' => 'Alta', 'utilidad_porcentaje' => 100]);

    $asc = $this->actingAs($user)
        ->getJson('/api/v1/articulos?sort=utilidad_porcentaje_efectivo&direction=asc');
    $asc->assertOk();
    expect(collect($asc->json('data'))->pluck('nombre')->all())->toBe(['Baja', 'Media', 'Alta']);

    $desc = $this->actingAs($user)
        ->getJson('/api/v1/articulos?sort=utilidad_porcentaje_efectivo&direction=desc');
    $desc->assertOk();
    expect(collect($desc->json('data'))->pluck('nombre')->all())->toBe(['Alta', 'Media', 'Baja']);
});

test('el listado ordena por utilidad porcentaje efectiva heredando la del catalogo (035)', function () {
    $user = User::factory()->create();
    $catalogo = Catalogo::factory()->for($user)->create(['utilidad_porcentaje' => 15]);

    Articulo::factory()->for($user)->for($catalogo)->create([
        'nombre' => 'Hereda del catalogo', 'utilidad_porcentaje' => null,
    ]);
    Articulo::factory()->for($user)->for($catalogo)->create([
        'nombre' => 'Utilidad propia', 'utilidad_porcentaje' => 50,
    ]);
    ...
    $response->assertOk();
    expect(collect($response->json('data'))->pluck('nombre')->all())
        ->toBe(['Hereda del catalogo', 'Utilidad propia']);
});

test('ordenar por utilidad porcentaje efectiva se combina con el filtro de nombre sin ambiguedad de columnas (035)', function () {
    // La subconsulta que resuelve este orden no debe volver ambigua la columna `nombre`, que
    // existe tanto en articulos como en catalogos.
    $user = User::factory()->create();
    $catalogo = Catalogo::factory()->for($user)->create();

    Articulo::factory()->for($user)->for($catalogo)->create(['nombre' => 'Sello redondo']);
    Articulo::factory()->for($user)->for($catalogo)->create(['nombre' => 'Sello cuadrado']);
    Articulo::factory()->for($user)->for($catalogo)->create(['nombre' => 'Almohadilla']);

    $response = $this->actingAs($user)->getJson(
        '/api/v1/articulos?filtro_nombre=Sello&sort=utilidad_porcentaje_efectivo&direction=asc'
    );

    $response->assertOk();
    expect(collect($response->json('data'))->pluck('nombre')->all())
        ->toBe(['Sello redondo', 'Sello cuadrado']);
});
